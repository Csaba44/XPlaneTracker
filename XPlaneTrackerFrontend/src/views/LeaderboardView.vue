  ...
  <div class="flex h-screen w-screen bg-flight-bg text-slate-300 overflow-hidden font-sans">
    ...
    <main class="flex-grow overflow-y-auto p-10 relative">
      <div class="max-w-6xl mx-auto">
        <h1 class="text-3xl font-black text-white italic mb-10 uppercase tracking-tighter"><i class="fa-solid fa-trophy text-flight-accent mr-3"></i> Top Landings Leaderboard</h1>
        ...
        <div v-else-if="leaderboard.length === 0" class="text-center py-20 text-slate-500 bg-flight-card/50 rounded-xl border border-flight-border border-dashed">
          <i class="fa-solid fa-ranking-star text-4xl mb-4 text-flight-accent opacity-50"></i>
          <p class="text-lg font-bold text-slate-400">Nincs elég adat a ranglistához.</p>
          <p class="text-[10px] uppercase tracking-widest mt-2">Legalább 2 pilóta landolása szükséges egy adott reptéren.</p>
        </div>

        <div v-for="airport in leaderboard" :key="airport.icao" class="mb-12 bg-flight-card border border-flight-border rounded-xl shadow-2xl overflow-hidden">
