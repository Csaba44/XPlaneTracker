...
<template>
  <div class="flex flex-col space-y-2 mb-4">
    ...
    <router-link v-if="route.name !== 'community'" to="/community" class="w-full flex justify-center items-center space-x-2 bg-teal-500/10 hover:bg-teal-500 text-teal-500 hover:text-white border border-teal-500 transition-colors p-2 rounded-lg text-xs font-bold uppercase tracking-wider cursor-pointer">
      <i class="fa-solid fa-earth-europe"></i>
      <span>Közösség</span>
    </router-link>

    <router-link v-if="route.name !== 'friends'" to="/friends" class="w-full flex justify-center items-center space-x-2 bg-amber-500/10 hover:bg-amber-500 text-amber-500 hover:text-white border border-amber-500 transition-colors p-2 rounded-lg text-xs font-bold uppercase tracking-wider cursor-pointer">
      <i class="fa-solid fa-users"></i>
      <span>Barátok</span>
    </router-link>

    <router-link v-if="user?.is_admin === 1" to="/admin" class="w-full text-center bg-purple-500/10 hover:bg-purple-500 text-purple-500 hover:text-white border border-purple-500 transition-colors p-2 rounded-lg text-xs font-bold uppercase tracking-wider block mt-2"> Admin Panel </router-link>
