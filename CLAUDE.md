...
- **`events`**: An array of state-machine events emitted by `flight_state.py`. Each entry has `ts` (Unix seconds) and `type`, plus type-specific extras. Types: `engine_start` / `engine_shutdown` (`engine`), `liftoff` (`pitch`, `max_pitch_10s`, `roll`, `ias`, `gs`, `alt`), `touchdown`, `gear_up` / `gear_down` (`alt`, `ias`), `flaps_set` (`index`), `stall` (`alt`, `ias`, `pitch`), `touch_and_go` (`gs`), `phase_change` (`phase`).
